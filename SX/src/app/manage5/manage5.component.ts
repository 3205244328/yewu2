...
export class Manage5Component {
  ...
  humion() {
    const xx = []
    this.httpClient.put(this.baseUrl + 'humi/humion', xx).subscribe(
      (val: any) => {  // val是服务器返回的值
        if (val.succ) {
          alert('加湿器打开');
        }
  ...
  humioff() {
    const xx = []
    this.httpClient.put(this.baseUrl + 'humi/humioff', xx).subscribe(
      (val: any) => {  // val是服务器返回的值
        if (val.succ) {
          alert('加湿器关闭');
        }
  ...
  heaton() {
    const xx = []
    this.httpClient.put(this.baseUrl + 'heat/heaton', xx).subscribe(
      (val: any) => {  // val是服务器返回的值
        if (val.succ) {
          alert('加热器打开');
        }
  ...
  heatoff() {
    const xx = []
    this.httpClient.put(this.baseUrl + 'heat/heatoff', xx).subscribe(
      (val: any) => {  // val是服务器返回的值
        if (val.succ) {
          alert('加热器关闭');
        }
